class VoxelTypeData {
	constructor() {
		...
		this.voxelTypes = new Uint8Array(voxelTypesBuffer);
		this.initializeVoxelTypes();	
	}
		...
		for(let x = 0; x < WORLD_SIZE.x; x++) {
			for(let z = 0; z < WORLD_SIZE.z; z++) {
				...
				for(let y = 0; y < WORLD_SIZE.y; y++) {
					if(y <= height - 2) {
						this.voxelTypes[voxelIndex] = 0;
					} else if(y <= height && y <= this.getSandHeightFromSimplex(x, z)) {
						this.voxelTypes[voxelIndex] = 2;
					} else if(y <= height && y > WATER_SURFACE_HEIGHT) {
						this.voxelTypes[voxelIndex] = 1;
					} else if(y > height && y <= WATER_SURFACE_HEIGHT) {
						this.voxelTypes[voxelIndex] = VOXEL_TYPE.WATER;						
					} else {
						this.voxelTypes[voxelIndex] = VOXEL_TYPE.AIR;
					}
					voxelIndex++;
				}
			}
	...
	getVoxelType(coords) {
		return this.voxelTypes[this.getIndexFromCoordinates(coords)];
	}

	getVoxelTypeByIndex(index) {
		return this.voxelTypes[index];
	}

	setVoxelType(coords, type) {
		this.voxelTypes[this.getIndexFromCoordinates(coords)] = type;
	}
