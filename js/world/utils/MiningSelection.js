class MiningSelection{
	...
	update(mousePicker) {
		if(keyboard.sTipped && mousePicker.getSelectedCubeCoords() !== undefined) {
			...
			if(index === undefined) {
				...
				this.assignedToDwarf.push(undefined);
				this.updateAllWalkableCoordsToReachSelectedCoords();
	...
	getWalkableCoordsToReachSelectedCoordsBySelectedCoord(coords) {
		for(let i = 0; i < this.selectedCoords.length; i++) {
			if( areCoordsEqual(coords, this.selectedCoords[i]) === true ) {
				return this.walkableCoordsToReachSelectedCoords[i];
			}
		}
	}
	...
	deAssignMiningCoords(coords) {
		iLoop: for(let i = 0; i < this.selectedCoords.length; i++) {
			if( areCoordsEqual(coords, this.selectedCoords[i]) === true ) {
				this.assignedToDwarf[i] = undefined;
				break iLoop;
			}
		}
	}
	...
	updateWalkableCoordsToReachSelectedCoordsBySelectedCoord(coords) {
		iLoop: for(let i = 0; i < this.selectedCoords.length; i++) {
			if( areCoordsEqual(coords, this.selectedCoords[i]) === true ) {
				this.walkableCoordsToReachSelectedCoords[i] = this.getWalkableCoordsToReachSelectedCoordsByIndex(i);
				break iLoop;
			}
		}		
	}
	...
	updateAllWalkableCoordsToReachSelectedCoords() {
		this.walkableCoordsToReachSelectedCoords = [];
		for(let i = 0; i < this.selectedCoords.length; i++) {
			this.walkableCoordsToReachSelectedCoords.push(this.getWalkableCoordsToReachSelectedCoordsByIndex(i));
		}
	}	
	...
	getWalkableCoordsToReachSelectedCoordsByIndex(index) {
		...
		yLoop: for(let y = -1; y <= 1; y++) {
			...
			for(let j = 0; j < faceNeighbors.length; j++) {
				let coordsNextToSelected = [
					this.selectedCoords[index][0] + faceNeighbors[j].x,
					this.selectedCoords[index][1] + y,
		...
		return walkableNeighborsToReachCurrentSelectedCube;
	}
